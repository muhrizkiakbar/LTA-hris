		...
		<div class="row">
			<div class="col-xl-12">
              ...
              <div class="text-center mb-2">
                <h4 class="text-uppercase" style="font-weight: 800"><u>Pengajuan Surat Ijin Meninggalkan Tempat Kerja</u></h4>
                <span style="font-size: 14px;">Nomor : LTA / SIM / {{ $thn }} / {{ $bln }} / {{ $get->employee->nik }} / {{ $get->kd }} </span>
              </div>
                ...
                <tr>
                  ...
                  <td>{{ $employee->no_hp }}</td>
								...
								<tr>
                  <td colspan="3" class="text-center"><strong>DETAIL SURAT IJIN MENINGGALKAN TEMPAT KERJA</strong></td>
                </tr>
                <tr>
                  <td class="text-center">1</td>
                  <td>Tanggal Ijin</td>
                  <td>{{ tgl_def($get->date) }} </td>
                </tr>
                <tr>
                  <td class="text-center">2</td>
                  <td>Jam</td>
                  <td>{{ $get->time_start }} s/d {{ $get->time_end }}</td>
                </tr>
                <tr>
                  <td class="text-center">3</td>
                  <td>Tipe Ijin</td>
                  <td>{{ $get->tipe_surat->title }}</td>
                </tr> 
                <tr>
                  <td class="text-center">4</td>
                  <td>Keperluan</td>
...
<script type="text/javascript">
	$(document).ready(function(){
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.surat.ijin_periksa') }}';
      ...
      $.ajax({
        ...
        success: function (response){
					if (response.message=='sukses') {
						var base = "{{ url('/backend/surat/ijin_detail/') }}";
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.surat.ijin_setuju') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.surat.ijin_ketahui') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.surat.ijin_reject') }}';
			var token = '{{ csrf_token() }}';
			Swal.fire({
