		...
		<div class="row">
					...
					<div class="card-body">
						<table class="table table-sm table-striped table-bordered datatable-basic">
							...
							<tbody>
								...
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ isset($item->employee) ? $item->employee->nik : '-' }}</td>
									<td>{{ isset($item->employee) ? $item->employee->name : '-' }}</td>
									<td>{{ $item->date_start }} - {{ $item->date_end }}</td>
									<td>{{ $item->keterangan }}</td>
									<td class="text-center">
										<a href="{{ route('backend.absensi.ijin_detail',$item->kd) }}" target="_blank">
											<span class="badge badge-success">Detail</span>
										</a>
										<a href="{{ route('backend.absensi.ijin_delete',$item->id) }}"  onclick="return confirm('Anda yakin ingin menghapus ?')">
			...
			<div class="col-xl-5">
				<div class="card">
					...
					<div class="card-body">
						{!! Form::open(['route'=>'backend.absensi.ijin_store','method'=>'POST','files' => true]) !!}
						@include('backend.absensi.ijin.form')
