			...
			<div class="col-12">
				<div class="page-title-box d-flex align-items-center justify-content-between">
					...
					<div class="page-title-right">
						...
						@endif
						@if (empty($get->status) || $get->status!=2)
							@if ($get->periksa_st==0)
							...
							@endif
							{{-- <a href="javascript:void(0);" class="btn btn-danger btn-sm reject mr-2" data-id="{{ $get->kd }}">
		...
		<div class="row">
			<div class="col-xl-12">
              ...
              <div class="text-center mb-2">
                <h4 class="text-uppercase" style="font-weight: 800"><u>Pengajuan Absensi Ijin Karyawan</u></h4>
                <span style="font-size: 14px;">Nomor : LTA / CT / {{ $thn }} / {{ $bln }} / {{ $get->employee->nik }} / {{ $get->kd }} </span>
              </div>
                ...
                <tr>
								...
								<tr>
									<td class="text-center">2</td>
									<td>Tipe Ijin</td>
									<td>{{ $get->absensi_ijin_type->title }} {{ isset($get->keterangan) ? '('.$get->keterangan.')' : '' }}</td>
								</tr>
								<tr>
									<td class="text-center">3</td>
									...
									<td>
										...
										-  
										@else
										<a href="{{ Storage::url('upload/employee/'.$get->employee->nik.'/'.$get->file) }}" target="_blank">
											{{ $get->file }}
										</a>
										@endif
									</td>
								</tr>
								@if ($get->reject_excuse!='')
								<tr>
									<td colspan="3" class="text-center text-danger"><strong>ALASAN REJECT</strong></td>
								</tr>
								<tr>
									<td colspan="3" class="text-danger">{{ $get->reject_excuse }}</td>
								</tr>
								@endif
              </table>
							<table class="table table-borderless table-sm">
								<tr>
									<td>{{ $get->employee->lokasi }}, {{ tgl_indo($get->date_start) }}</td>
								</tr>
							</table>
              <table class="table table-borderless table-sm" width="100%">
								...
								<tr>
									<td class="text-center" width="25%">
										<img src='{{ get_ttd($get->users_id) }}' height="70px">
									</td>
									<td class="text-center" width="25%">
										@if ($get->periksa_st==1) 
											<img src='{{ get_ttd($get->periksa_id) }}' height="70px">
										@endif
									</td>
									<td class="text-center" width="25%">
										@if ($get->approval1_st==1) 
											<img src='{{ get_ttd($get->approval1_id) }}' height="70px">
										@endif
									</td>
									<td class="text-center" width="25%">
										@if ($get->approval2_st==1) 
											<img src='{{ get_ttd($get->approval2_id) }}' height="70px">
										@endif
									</td>
								</tr>
								<tr>
									<td class="text-center" width="25%">
										<u>{{ $get->employee->name }}</u> <br> 
										{{ isset($get->jabatan_id) ? $get->department_jabatan->title : '-' }}
									</td>
									<td class="text-center" width="25%">                 
									...
									<td class="text-center" width="25%">
										...
										@else
										{{ isset($get->approval_first->department_jabatan_id) ? $get->approval_first->jabatan->title : '-' }}    
										@endif
									</td>
									<td class="text-center" width="25%">
										...
										@else
										{{ isset($get->approval_second->department_jabatan_id) ? $get->approval_second->jabatan->title : '-' }}    
										@endif
									</td>
								</tr>
							</table>
...
<script type="text/javascript">
	$(document).ready(function(){
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.absensi.ijin_periksa') }}';
      ...
      $.ajax({
        ...
        success: function (response){
					if (response.message=='sukses') {
						var base = "{{ url('/backend/absensi/ijin_detail') }}";
            var href = base+"/"+response.id;
						$('#overlay').hide();
						Swal.fire({
              icon: 'success',
              type: 'success',
              title: 'Approve dokumen berhasil !',
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.absensi.ijin_setuju') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.absensi.ijin_ketahui') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.absensi.ijin_reject') }}';
			var token = '{{ csrf_token() }}';
			Swal.fire({
