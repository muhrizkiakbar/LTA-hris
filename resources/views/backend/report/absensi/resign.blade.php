					...
					<div class="card-body">
						<form method="POST" id="search">
							<div class="form-group row">
								...
								<div class="col-md-2">{!! Form::text('periode',null,['class'=>'form-control datepick','id'=>'periode','data-language'=>'en','required'=>true,'data-min-view'=>'months','data-view'=>'months','data-date-format'=>'MM yyyy']) !!}</div>
								<div class="col-md-4">
...
<script type="text/javascript">
  $(document).ready(function(){
      ...
      var url = '{{ route('backend.report.absensi.resign_generate') }}';
      $('#overlay').fadeIn();
      $.ajax({
        url : url,
      ...
      var periode = $("#periode").val();
			var token = '{{ csrf_token() }}';
      var url = '{{ route('backend.report.absensi.resign_search') }}';
			$('#overlay').fadeIn();
			$.ajax({
        url : url,
        data  : {department:department, lokasi:lokasi, periode:periode, _token:token},
      ...
      var tF = 'Report Absensi Monthly.xls'; //Filename
