						...
						<table class="table table-sm table-striped table-bordered datatable-basic">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ $item->lokasi_asal->title }}</td>
									<td>{{ $item->lokasi_tujuan->title }}</td>
									<td class="text-right">{{ rupiahnon($item->uang_makan) }}</td>
									<td class="text-right">{{ rupiahnon($item->uang_hotel) }}</td>
									<td class="text-center">
										<a href="{{ route('backend.master.dinas_except.delete',$item->id) }}" onclick="return confirm('Anda yakin ingin menghapus ?')">
			...
			<div class="col-xl-5">
				<div class="card">
					<div class="card-header d-flex align-items-center justify-content-between">
						<h6 class="card-title">Tambah Data - {{ $title }}</h6>
					</div>
					<div class="card-body">
						{!! Form::open(['route'=>'backend.master.dinas_except.store','method'=>'POST','files'=>true]) !!}
              @include('backend.master.dinas_except.form')
