		...
		<div class="row">
			<div class="col-xl-12">
              ...
              <div class="text-center mb-2">
                <h4 class="text-uppercase" style="font-weight: 800"><u>Pengajuan Cuti Khusus</u></h4>
                <span style="font-size: 14px;">Nomor : LTA / CK / {{ $thn }} / {{ $bln }} / {{ $get->employee->nik }} / {{ $get->kd }} </span>
              </div>
...
<script type="text/javascript">
	$(document).ready(function(){
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.cuti.khusus.periksa') }}';
      ...
      $.ajax({
        ...
        success: function (response){
					if (response.message=='sukses') {
						var base = "{{ url('/backend/cuti/khusus/detail') }}";
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.cuti.khusus.setuju') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.cuti.khusus.ketahui') }}';
			var token = '{{ csrf_token() }}';
			$('#overlay').fadeIn();
      $.ajax({
        url: url,
      ...
      var id = $(this).data('id');
      var url = '{{ route('backend.cuti.khusus.reject') }}';
			var token = '{{ csrf_token() }}';
			Swal.fire({
