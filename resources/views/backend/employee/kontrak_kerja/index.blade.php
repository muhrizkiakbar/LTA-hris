			...
			<div class="col-12">
				<div class="page-title-box d-flex align-items-center justify-content-between">
					...
					<div class="page-title-right">
						<a href="javascript:void(0);" class="btn btn-primary btn-sm create">Tambah Data</a>
					</div>
				</div>
			</div>
		</div>
						...
						<table class="table table-sm table-striped table-bordered datatable-basic">
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ $item->nik }}</td>
									<td>{{ $item->name }}</td>
									<td class="text-center">{{ $item->tgl_start }}</td>
									<td class="text-center">{{ $item->tgl_end }}</td>
									<td>{{ $item->status }}</td>
									<td class="text-center">
										<a href="{{ route('backend.employee.kontrak_kerja.delete',$item->id) }}" onclick="return confirm('Anda yakin ingin menghapus ?')">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var oTable = $('.datatable-basic').DataTable({
      ...
      var url = '{{ route('backend.employee.kontrak_kerja.create') }}';
