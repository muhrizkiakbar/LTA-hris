...
class KhususServices
{
    ...
    $role = auth()->user()->role_id;
		...
		{
			$get = CutiHeader::where('cuti_type_id',2)
    ...
    $role = auth()->user()->role_id;
		...
		$khusus = CutiKhusus::pluck('title','id');
    ...
    $employee_exchange = $data['employee_exchange_id'];
		$cuti_khusus = $data['cuti_khusus_id'];

		$getkhusus = CutiKhusus::find($cuti_khusus);
    $khusus_title = $getkhusus->title;
    ...
    $tgl_end = $split[$count_tgl];
		...
		$cek_header = CutiHeader::where('cuti_type_id',2)
														->where('approval',0)
